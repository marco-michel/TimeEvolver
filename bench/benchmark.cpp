...
std::string backendToString(backendMode backend)
{
    switch (backend) {
...
#ifdef USE_CUDA
    const long double bytesPerValue =
        sizeof(cuDoubleComplex) + sizeof(std::int64_t);
    const long double rowOffsetBytes =
        static_cast<long double>(rows + 1U) * sizeof(std::int64_t);
#else
    const long double bytesPerValue =
...
{
    return { "simple", "blackhole_lb", "blackhole_medium", "blackhole_large", "blackhole_gpu_quick", "blackhole_gpu_1g", "blackhole_gpu_2g", "custom" };
}
...
std::string queryGpuName()
{
    ...
    const cudaError_t propStatus = cudaGetDeviceProperties(&properties, 0);
...
struct gpuSpmvTiming {
    timingSummary compute;
    timingSummary endToEnd;
    double maxError = 0.0;
};

gpuSpmvTiming runGpuSpmvBenchmark(const benchmarkCase& input, int warmup, int repeats)
{
    ...
    std::vector<std::complex<double>> referenceOutput(input.hamiltonian->n, std::complex<double>(0.0, 0.0));

    input.hamiltonian->spMV(smatrix::one, hostInput.data(), referenceOutput.data());
    TE_CUDA_CHECK(cudaMemcpy(gpuMatrix.CX, hostInput.data(), hostInput.size() * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
    ...
    computeSamples.reserve(repeats);
    endToEndSamples.reserve(repeats);

    cudaEvent_t startEvent = nullptr;
    cudaEvent_t stopEvent = nullptr;
    TE_CUDA_CHECK(cudaEventCreate(&startEvent));
    TE_CUDA_CHECK(cudaEventCreate(&stopEvent));

    for (int i = 0; i < repeats; ++i) {
        TE_CUDA_CHECK(cudaEventRecord(startEvent));
        gpuMatrix.spMV(gpuMatrix.oneCUDA, gpuMatrix.vecX, gpuMatrix.vecY);
        TE_CUDA_CHECK(cudaEventRecord(stopEvent));
        TE_CUDA_CHECK(cudaEventSynchronize(stopEvent));

        float elapsedMs = 0.0f;
        TE_CUDA_CHECK(cudaEventElapsedTime(&elapsedMs, startEvent, stopEvent));
        computeSamples.push_back(static_cast<double>(elapsedMs));

        const auto wallStart = std::chrono::steady_clock::now();
        TE_CUDA_CHECK(cudaMemcpy(gpuMatrix.CX, hostInput.data(), hostInput.size() * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice));
        gpuMatrix.spMV(gpuMatrix.oneCUDA, gpuMatrix.vecX, gpuMatrix.vecY);
        TE_CUDA_CHECK(cudaMemcpy(hostOutput.data(), gpuMatrix.CY, hostOutput.size() * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost));
        const auto wallStop = std::chrono::steady_clock::now();

        const std::chrono::duration<double, std::milli> wallElapsed = wallStop - wallStart;
        endToEndSamples.push_back(wallElapsed.count());
    }

    TE_CUDA_CHECK(cudaEventDestroy(startEvent));
    TE_CUDA_CHECK(cudaEventDestroy(stopEvent));

    double maxError = 0.0;
    for (std::size_t i = 0; i < hostOutput.size(); ++i) {
        maxError = std::max(maxError, std::abs(hostOutput[i] - referenceOutput[i]));
    }

    gpuSpmvTiming result;
    result.compute = summarizeSamples(computeSamples);
    result.endToEnd = summarizeSamples(endToEndSamples);
...
{
public:
    arnoldiBenchmarkHarness(const benchmarkCase& input, executionBackend backend)
        : krylovTimeEvolver(
            input.maxT,
    ...
    void runArnoldiStep()
    {
        ...
        if (useCUDA) {
            arnoldiAlgorithmCUDA(tolRate, &hessenberg, &basis, &h, &actualKrylovDim);
        } else {
...
benchmarkRecord runArnoldiBenchmark(const benchmarkCase& input, int warmup, int repeats, krylovTimeEvolver::executionBackend backend, const std::string& backendLabel)
{
    arnoldiBenchmarkHarness harness(input, backend);
...
benchmarkRecord runE2EBenchmark(const benchmarkCase& input, int warmup, int repeats, krylovTimeEvolver::executionBackend backend, const std::string& backendLabel, bool includeObservables)
{
...
std::vector<benchmarkRecord> describeCase(const benchmarkCase& input, backendMode backend)
{
    ...
    if (backend == backendMode::gpu || backend == backendMode::both) {
        ...
        try {
            ...
            benchmarkRecord gpuRecord = makeDescriptionRecord(input, "gpu");
            const double workVectorGiB = bytesToGiB(
                static_cast<long double>(input.hamiltonian->n + input.hamiltonian->m) *
                sizeof(cuDoubleComplex));
            const double bufferGiB = bytesToGiB(static_cast<long double>(gpuMatrix.bufferSize));
            gpuRecord.note =
                "gpu_name=" + queryGpuName() +
                " cuda_initialized=1" +
                " work_vectors_gib=" + std::to_string(workVectorGiB) +
                " spmv_buffer_gib=" + std::to_string(bufferGiB) +
                " matrix assembly is still host-side";
            records.push_back(std::move(gpuRecord));
        }
...
std::vector<benchmarkRecord> runBenchmarks(const benchmarkCase& input, benchmarkKind benchmark, backendMode backend, int warmup, int repeats, bool includeObservables)
{
    ...
    const auto maybeRunCpu = [&]() {
        ...
        case benchmarkKind::spmv:
            ...
            return;
        case benchmarkKind::arnoldi:
            records.push_back(runArnoldiBenchmark(input, warmup, repeats, krylovTimeEvolver::executionBackend::CPU, "cpu"));
            return;
        case benchmarkKind::e2e:
            records.push_back(runE2EBenchmark(input, warmup, repeats, krylovTimeEvolver::executionBackend::CPU, "cpu", includeObservables));
            return;
        }
    };
    ...
    const auto maybeRunGpu = [&]() {
        ...
        case benchmarkKind::spmv: {
            ...
            record.benchmark = "spmv";
            record.caseName = input.name;
            record.backend = "gpu";
            record.dimension = input.hamiltonian->n;
            record.nnz = input.hamiltonian->numValues;
            record.krylovDim = input.krylovDim;
            record.warmup = warmup;
            record.repeats = repeats;
            populateStaticMetrics(record);

            const gpuSpmvTiming timing = runGpuSpmvBenchmark(input, warmup, repeats);
            record.wall = timing.endToEnd;
            record.deviceMeanMs = timing.compute.meanMs;
            record.transferMeanMs = timing.endToEnd.meanMs - timing.compute.meanMs;
            record.maxError = timing.maxError;
            record.note = std::string("gpu_name=") + queryGpuName();
            records.push_back(record);
            return;
        }
        case benchmarkKind::arnoldi:
            records.push_back(runArnoldiBenchmark(input, warmup, repeats, krylovTimeEvolver::executionBackend::GPU, "gpu"));
            records.back().note = std::string("gpu_name=") + queryGpuName();
            return;
        case benchmarkKind::e2e:
            records.push_back(runE2EBenchmark(input, warmup, repeats, krylovTimeEvolver::executionBackend::GPU, "gpu", includeObservables));
            records.back().note = std::string("gpu_name=") + queryGpuName();
            return;
        }
#else
...
int main(int argc, char* argv[])
{
    ...
    options.add_options()
        ...
        ("backend", po::value<std::string>(&backendArg)->default_value("both"), "Backend to run: cpu, gpu, both")
        ("case", po::value<std::string>(&caseArg)->default_value("simple"), "Benchmark case: simple, blackhole_lb, blackhole_medium, blackhole_large, blackhole_gpu_quick, blackhole_gpu_1g, blackhole_gpu_2g, custom")
